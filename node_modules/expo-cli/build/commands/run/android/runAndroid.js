...
async function resolveOptionsAsync(projectRoot, options) {
  ...
  let port = options.bundler ? await (0, _resolvePortAsync().resolvePortAsync)(projectRoot, options.port) : null;
...
async function runAndroidActionAsync(projectRoot, options) {
  ...
  schemes.length) {
    ...
    _log().default.debug('Opening app on device via package name: ' + props.device.name); // For now, just open the app with a matching package name
