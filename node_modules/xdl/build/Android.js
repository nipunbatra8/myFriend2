...
async function installExpoAsync({
  ...
  const path = await (0, _internal().downloadApkAsync)(url, progress => bar.tick(1, progress));
  ...
  if (version) {
    _internal().Logger.global.info(`Installing Expo Go ${version} on device`);
  } else {
    _internal().Logger.global.info(`Installing Expo Go on device`);
  }
