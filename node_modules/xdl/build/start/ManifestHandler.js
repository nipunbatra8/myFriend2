...
async function getManifestResponseAsync({
  ...
  manifest.debuggerHost = await _internal().UrlUtils.constructDebuggerHostAsync(projectRoot, hostname);
  manifest.logUrl = await _internal().UrlUtils.constructLogUrlAsync(projectRoot, hostname);
  manifest.hostUri = await _internal().UrlUtils.constructHostUriAsync(projectRoot, hostname); // Resolve all assets and set them on the manifest as URLs
